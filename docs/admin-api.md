...
## 18. Token Usage Records (Admin 新增)

前端路由新增：`/usage-records`

新增接口如下（均为分页返回 `Result<PageResult<T>>`）：

- `GET /api/usage-records`
- `GET /api/usage-records/summary/account`
- `GET /api/usage-records/summary/model`

公共查询参数：

- `page` / `pageSize`
- `accountKeyword`（用户名 / 手机号 / 邮箱模糊匹配）
- `modelCode`
- `providerCode`
- `usageType`（`balance` / `plan`）
- `startAt`、`endAt`（格式：`yyyy-MM-dd HH:mm:ss`）
...
- 账户汇总、模型汇总均返回 `avgSessionMs`（平均会话耗时，毫秒）
- 统计字段包含 `promptTokens`、`cacheHitTokens`、`completionTokens`、`totalTokens`、`totalCost`、`requestCount`
- 流水记录返回 `responseMs`（首次响应时间，毫秒）与 `sessionMs`（会话耗时，毫秒）

套餐管理 UI 交互补充：

- 新增/编辑套餐时，“可用模型范围”支持“全选 / 全不选”操作
