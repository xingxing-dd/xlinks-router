...
`POST /api/v1/orders` 在落库层对应 `customer_orders`，核心字段如下：

- `order_no`：平台订单号（接口 `orderId`）
- `ref_no`：支付渠道侧关联号（如支付宝 `trade_no`）
- `account_id`：下单用户 ID
- `order_type`：订单类型（充值/提现/购买订阅等）
- `order_title`：订单标题
- `order_info`：JSON 快照（业务目标、套餐快照等）
- `payment_channel`：支付通道
- `total_amount`：订单金额
- `status`：`0待支付` `1支付成功` `2支付失败` `3已关闭` `4已退款`
- `complete_at`：完成时间
- `expired_at`：支付过期时间（默认创建后 30 分钟）
- `remark`：备注
...
- 订单创建后默认 30 分钟过期
- 到达 `expired_at` 仍未支付（`status=0`）时，自动置为 `3已关闭`
- 支付回调成功后会按 `order_type` 执行业务履约；`subscription_purchase` 会基于 `order_info` 快照生成 `customer_plans`
- 激活码兑换（`POST /api/v1/activation-codes/consume`）会同步落库 `customer_orders`，并标记 `payment_channel=third-party`
