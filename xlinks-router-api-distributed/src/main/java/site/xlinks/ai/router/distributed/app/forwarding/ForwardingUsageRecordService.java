...
public class ForwardingUsageRecordService {
    ...
    private void settleUsage(ForwardingUsageContext context, UsageRecord record) {
        ...
        try {
            walletService.debitBasicAllowOverdraftToZero(
            ...
            if (isAllowedBasicWalletOverdraft(ex)) {
                log.warn("余额扣减发生透支保护。requestId={}, accountId={}, totalCost={}",
                        context.getRequestId(), context.getAccountId(), record.getTotalCost());
                return;
            }
            throw ex;
        }
