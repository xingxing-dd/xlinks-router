...
public class RouteCacheService {
    ...
    private final Map<Long, ProviderToken> providerTokenByIdCache = new ConcurrentHashMap<>();
    ...
    public void refreshModels() {
        ...
        for (Model model : models) {
            ...
            modelCache.put(model.getModelCode(), model);
        }
        log.debug("Model cache loaded, size={}", modelCache.size());
    }
    ...
    public void refreshProviderTokens() {
        ...
        for (ProviderToken token : tokens) {
            ...
            providerTokenCache
                    ...
                    .add(token);
            if (token.getId() != null) {
                providerTokenByIdCache.put(token.getId(), token);
            }
        }
    ...
    public List<ProviderModel> getProviderModels(Long modelId) {
        ...
        if (providerModels == null || providerModels.isEmpty()) {
            providerModelCache.put(modelId, Collections.emptyList());
            return Collections.emptyList();
        }
        List<ProviderModel> copy = new ArrayList<>(providerModels);
        providerModelCache.put(modelId, copy);
        return copy;
    }
    ...
    public List<ProviderToken> getProviderTokens(Long providerId) {
        ...
        if (tokens == null || tokens.isEmpty()) {
            providerTokenCache.put(providerId, Collections.emptyList());
            return Collections.emptyList();
        }
