...
public class OpenAICompatibleAdapter implements ChatProviderAdapter {
    ...
    public ChatCompletionResponse chatCompletion(ChatCompletionRequest request, ProviderInvokeContext context) {
        try {
            ...
            // 构建请求体（将模型名替换为底层模型名）
            ChatCompletionRequest adaptedRequest = adaptRequest(request, context.getProviderModel());
     ...
     */
    private ChatCompletionRequest adaptRequest(ChatCompletionRequest request, String providerModel) {
        // 创建副本并替换模型名
        ...
        adapted.setStream(request.getStream());
