...
class ProviderConcurrencyGuardTest {
    ...
    @Test
    void shouldIgnoreAutoRenewSchedulingWhenSchedulerRejectsTask() {
        Provider provider = provider();
        ProviderToken token = token();
        ProviderPermitLease lease = new ProviderPermitLease(
                provider.getId(),
                token.getId(),
                token.getTokenName(),
                "permit-1",
                new ProviderRuntimePolicy(true, 2, 500, 40000, 40000, 30000, 30000, 15000)
        );
        when(renewScheduler.scheduleAtFixedRate(any(Runnable.class), any(Duration.class)))
                .thenThrow(new TaskRejectedException("scheduler is shutting down"));

        ScheduledFuture<?> future = assertDoesNotThrow(() ->
